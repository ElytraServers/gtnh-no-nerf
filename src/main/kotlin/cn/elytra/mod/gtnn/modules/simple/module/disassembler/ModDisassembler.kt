	...
	override fun registerGregTechItems() {
		...
		for((tier, mteId) in 1..5 withOffsetStartBy offsetBasicVersion) {
			val stack = MTEDisassembler(
				mteId,
				IModule.Companion.getPreferredTieredMachineLocalizationKey(tier, "disassembler"),
				IModule.Companion.getPreferredTieredMachineNameRegional(tier, "Disassembler", "Deconstructor"),
				tier,
			).getStackForm(1)
