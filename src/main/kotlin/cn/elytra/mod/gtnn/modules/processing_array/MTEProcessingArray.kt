...
class MTEProcessingArray : MTEExtendedPowerMultiBlockBase<MTEProcessingArray>, ISurvivalConstructable {
	...
	override fun getTexture(
		...
		if(side == facing) {
			return if(active) arrayOf(
				...
				buildTexture {
					addIcon(Textures.BlockIcons.OVERLAY_FRONT_PROCESSING_ARRAY_ACTIVE)
					extFacing()
				},
				buildTexture {
					addIcon(Textures.BlockIcons.OVERLAY_FRONT_PROCESSING_ARRAY_ACTIVE_GLOW)
					extFacing()
					glow()
				},
			)
				...
				Textures.BlockIcons.casingTexturePages[0][48],
				buildTexture {
					addIcon(Textures.BlockIcons.OVERLAY_FRONT_PROCESSING_ARRAY)
					extFacing()
				},
				buildTexture {
					addIcon(Textures.BlockIcons.OVERLAY_FRONT_PROCESSING_ARRAY_GLOW)
					extFacing()
					glow()
				},
			)
		...
		return object : ProcessingLogic() {
			override fun validateRecipe(recipe: GTRecipe): CheckRecipeResult {
				...
				if(GTMod.gregtechproxy.mLowGravProcessing && recipe.mSpecialValue == -100
					&& !MTEBasicMachine.isValidForLowGravity(recipe, baseMetaTileEntity.world.provider.dimensionId)
				) {
	...
	override fun onScrewdriverRightClick(
		...
		if(aPlayer.isSneaking) {
			super.onScrewdriverRightClick(side, aPlayer, aX, aY, aZ, itemStack)
		} else {
