...
public class TileEntityDisassembler extends GT_MetaTileEntity_BasicMachine {
	...
	private static final Object[][] OreDictionaryOverride = {{
		...
		// ItemStack to replace
		new ItemStack(Blocks.planks), new ItemStack(Blocks.cobblestone), new ItemStack(Items.diamond), new ItemStack(Blocks.log), new ItemStack(Items.stick), new ItemStack(Blocks.sapling)}};
	...
	public int checkRecipe() {
		...
		if(is.getItem() instanceof GT_MetaGenerated_Tool || isCircuit(is) || blackList.stream().anyMatch(t -> GT_Utility.areStacksEqual(t.toStack(), is, true)) || compareToUnpacker(is))
	...
	private Integer handleHardOverride(ItemStack is) {
		...
		for(GT_ItemStack stack : stacks) {
			...
			if(stack.isStackEqual(in) && stack.mStackSize <= is.stackSize) {
				return setOutputsAndTime(outputHardOverrides.get(stack).toArray(new ItemStack[0]), stack.mStackSize) ? FOUND_AND_SUCCESSFULLY_USED_RECIPE : DID_NOT_FIND_RECIPE;
			}
		}
		return null;
	}
	...
	private int checkRecipeMap() {
		GT_Recipe gt_recipe = NNRecipe.DisassemblerRecipes.findRecipe(this.getBaseMetaTileEntity(), true, this.maxEUInput(), null, this.getAllInputs());
		...
		if(gt_recipe.isRecipeInputEqual(false, null, this.getAllInputs())) {
			if(gt_recipe.mSpecialValue == -100) {
				...
				return setOutputsAndTime(gt_recipe.mOutputs, gt_recipe.mInputs[0].stackSize) ? FOUND_AND_SUCCESSFULLY_USED_RECIPE : FOUND_RECIPE_BUT_DID_NOT_MEET_REQUIREMENTS;
			}
		}
	...
	private static void handleRecipeTransformation(ItemStack[] inputs, ItemStack[] output, List<? extends GT_Recipe> recipesColl) {
		for(int i = 0, inputsLength = inputs.length; i < inputsLength; i++) {
			...
			if(recipesColl != null) inputsStacks = recipesColl.stream().map(x -> x.mInputs).collect(Collectors.toSet());
	...
	private Collection<DissassembleReference> findRecipeFromMachine() {
		...
		return possibleRecipes.stream().sorted(Comparator.comparingDouble(TileEntityDisassembler::getCheaperInputs)).collect(Collectors.toList());
	}

	private static double getCheaperInputs(TileEntityDisassembler.DissassembleReference x) {
